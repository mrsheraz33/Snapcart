...
function DeliveryBoyDashboard({ earning }: { earning: number }) {
  ...
  useEffect((): any => {
    ...
    socket.on("new-assignment", (deliveryAssignment) => {
      setAssignment((prev) => {
        ...
        const exists = prev.some((item) => item._id === deliveryAssignment._id);
  ...
  const handelReject = async (id: string) => {
    try {
      ...
      await axios.post(`/api/delivery/assignment/${id}/reject-assignment`).catch(() => {});
      setAssignment((prev) => prev.filter((item) => item._id !== id));
    } catch (error) {
  ...
  if (!activeOrder && assignment.length === 0) {
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-linear-to-b from-white to-green-50 p-6">
        <div className="max-w-md w-full text-center">
          ...
          <div className="bg-white border rounded-xl p-6 shadow-sm">
            ...
            <p className="mt-4 text-lg font-bold text-green-700">{earning} RS. Earned today</p>
            <button
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="mt-6 bg-white rounded-xl border shadow p-6">
            {!activeOrder.order.deliveryOtpVerification && !showOtpBox && (
              <button
                className="w-full py-4 bg-green-600 text-white rounded-lg cursor-pointer flex items-center justify-center font-medium hover:bg-green-700 transition-all"
                onClick={sendOtp}
              >
                {sendOtpLoading ? (
                  <Loader className="animate-spin text-white flex items-center" size={16} />
                ) : (
                  "Mark as Delivered"
                )}
              </button>
            ...
            {showOtpBox && (
              <div className="mt-4">
                <input
                  ...
                  className="w-full py-3 border rounded-lg text-center font-semibold text-lg"
                  maxLength={4}
                  onChange={(e) => setOtp(e.target.value)}
                  value={otp}
                />
                <button
                  className="w-full mt-4 bg-blue-600 text-white py-3 rounded-lg flex items-center justify-center font-medium hover:bg-blue-700 transition-all"
                  onClick={verifyOtp}
                >
                  {verifyOtpLoading ? (
                ...
                {otpError && <div className="text-red-600 mt-2 text-center text-sm">{otpError}</div>}
              </div>
            )}

            {activeOrder.order.deliveryOtpVerification && (
              <div className="text-green-700 text-center font-semibold">Delivery completed!</div>
            )}
          </div>
        </div>
      </div>
  ...
  return (
    <div className="w-full min-h-screen bg-gray-50 p-4">
      <div className="max-w-3xl mx-auto">
        <h2 className="text-2xl font-bold mt-30 mb-7.5">Delivery Assignments</h2>
        {assignment.map((a, index) => (
          <div key={index} className="p-5 bg-white rounded-xl shadow mb-4 border">
            <p className="font-semibold text-gray-800">Order Id #{a?.order._id.slice(-6)}</p>
            <p className="text-gray-600 text-sm mt-1">{a?.order?.address?.fullAddress}</p>
